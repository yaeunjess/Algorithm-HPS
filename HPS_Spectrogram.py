...
def PitchSpectralHps(X, freq_buckets, f_s, buffer_rms):
    ...
    print("X.shape[0] :", X.shape[0]) # fft 거친 후 나온 frquency 총 개수
    print("iLen :", iLen) # fft 거친 후 나온 frquency 총 개수를 iOrder로 나눈거
    afHps = X[np.arange(0, iLen)]
    print("afHps :", afHps)
    k_min = int(round(f_min / f_s * 2 * (X.shape[0] - 1)))  # 최소 주파수에 대한 인덱스 계산
    print("k_min :", k_min)
...
def main():
    ...
    # 코드 1개의 스펙트로그램 데이터를 준비하기 위한 배열 초기화
    spectrogram_data = []
    ...
    for chunk in buffer_chunks[0: 60]:
        ...
        print("Chunk 내 sample 수:", len(chunk))
                
        fft_freq, fft_res, fft_res_len = getFFT(chunk, len(chunk))
        fft_res = remove_dc_offset(fft_res)

        # 각 chunk의 스펙트로그램 데이터를 준비하기 위한 배열 초기화
        spectrogram_chunk_data = []
        # numpy 배열로 변환 (시간 축은 첫 번째 차원, 주파수 축은 두 번째 차원)
        spectrogram_chunk_data.append(np.abs(fft_res))
        spectrogram_data.append(spectrogram_chunk_data)
        ...
        ## show the graph of the result of the FFT with the correct frequencies in the legend. 
        N = fft_res_len                         # frequency 개수 반환
        fft_freq_interval = fft_freq[: N // 4]  # frequency 배열을 0~N//4 까지 반환
        fft_res_interval = fft_res[: N // 4]    # magnitude 배열을 0~N//4 까지 반환
        fig, ax = plt.subplots()
        ...
        print("각 chunk 별 전치 변경 전 : ", spectrogram_chunk_data)
        # numpy 배열로 변환 (시간 축은 첫 번째 차원, 주파수 축은 두 번째 차원)
        spectrogram_chunk_data = np.array(spectrogram_chunk_data).T  # 전치하여 시간이 열로, 주파수가 행으로 오도록 조정
        print("각 chunk 별 전치 변경 전 : ", spectrogram_chunk_data)
        print(len(spectrogram_chunk_data))
            
        ## 각 chunk의 스펙트로그램 그리기
        plt.figure(figsize=(10, 6))
        extent = [0, len(chunk), 200, 600] # extent는 [x축 시작 시간, x축 종료 시간, y축 시작 주파수, y축 종료 주파수]를 의미
        plt.imshow(spectrogram_chunk_data, aspect='auto', extent=extent, origin='lower', cmap='jet')

        plt.title('Spectrogram by a Chunk')
    ...
    print("전치 변경 전 : ", spectrogram_data)
    # 각 배열을 열 벡터로 변환하고 모두 이어붙임
    spectrogram_data = np.concatenate([chunk[0][:, np.newaxis] for chunk in spectrogram_data], axis=0)
    # numpy 배열로 변환 (시간 축은 첫 번째 차원, 주파수 축은 두 번째 차원)
    # spectrogram_data = np.array(spectrogram_data).T  # 전치하여 시간이 열로, 주파수가 행으로 오도록 조정
    print("전치 변경 후 : ", spectrogram_data) 
    print(len(spectrogram_data))
    
    ## 코드 1개의 스펙트로그램 그리기
    plt.figure(figsize=(10, 6))        
    extent = [0, len(input_buffer), 200, 600] # extent는 [x축 시작 시간, x축 종료 시간, y축 시작 주파수, y축 종료 주파수]를 의미
    plt.imshow(spectrogram_data, aspect='auto', extent=extent, origin='lower', cmap='jet')
